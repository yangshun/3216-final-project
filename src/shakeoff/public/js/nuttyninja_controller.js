...
socket.on('screen-echo', function(data) {
	if (data.action == 'join') {
		if (data.status.success) {
			console.log('YESSS !!!');
			$('#canpic').prepend('<img id="can" src="'+data.status.obj+'" />');
		} else {
...
function deviceMotionHandler(eventData) {
	...
	info = info.replace("Y", acceleration.y);
	info = info.replace("Z", acceleration.z);
	var force = Math.abs(acceleration.x) + Math.abs(acceleration.y) + Math.abs(acceleration.z);
	...
	if (force > 30) {
	    socket.emit('controller-input', {name: myname, key: force, action:'shake'});
	}
}
